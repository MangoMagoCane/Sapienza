...
def func2(D: dict[int, list[str]]) -> list[str]:
    ...
    W: list[str] = []
    for K in D.keys():
        W.append(sorted(D[K], reverse=True)[K])
    return W
...
def func4(string_list1: list[str], string_list2: list[str]) -> list[str]:
    ...
    return sorted(list(str_set), key=lambda s: (-len(s), s))
...
def func5(points: list[tuple[int, int]]) -> tuple[float, float]:
    # scrivi qui il tuo codice
    pts = sorted(points, key=lambda p: sqrt(p[0] ** 2 + p[1] ** 2))[:3]
    x: float = 0
    y: float = 0
    for p in pts:
        x += p[0]
...
def func6(text: str) -> dict[str, int]:
    dct: dict[str, int] = {}
    words: list[str] = [w.lower() for w in text.split(" ")]
    for word in words:
        if dct.get(word[0], -1) == -1:
            dct.update({word[0]: 0})
    for word in words:
        used_letters: list[str] = []
        for c in word:
            if dct.get(c, -1) != -1 and c not in used_letters:
                dct[c] += 1
