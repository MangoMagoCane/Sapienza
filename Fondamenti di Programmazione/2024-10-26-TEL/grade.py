...
def test_func1_4():
    ...
    """
    a_dict = {
        "capslock": ["electrocuted", "mistake", "hobbledehoys", "superadded"],
        "unhindered": ["arts"],
        "backlist": [
            "unordered",
            "hobbledehoys",
    ...
    if res != expected:
        ...
        if type(res) == dict:
            for k in expected:
                if expected[k] != res[k]:
                    my_print(
                        f"""[ERROR] Ad esempio, il carattere {k} si trova in {expected[k]} parole invece che in {res[k]}."""
                    )
                    my_print(
                        f"""[ERROR] For example, char {k} appears in {expected[k]} words instead than in {res[k]}."""
                    )
                    break
        return 0
    return 1
