...
def func1(testo: str, K: int) -> set[str]:
    ...
    parole = testo.split()
    return {p for p in parole if parole.count(p) == K}
...
L'ordinamento delle parole nei valori del risultato è per dimensioni 
decrescenti -> crescente
...
e ritornare un dizionario che ha come chiavi le parole presenti nel file 
"almeno K volte" -> "su almeno K righe"
...
def func4(path_in: str, path_out: str, K: int) -> dict[str, list[str]]:
    ...
    D = {}
    # completa la funzione
    with open(path_in, encoding="utf8") as FIN:
        for i, riga in enumerate(FIN):
            parole = riga.split()
            for parola in parole:
                if parola in D:
                    if i not in D[parola]:
                        D[parola].append(i)
                else:
                    D[parola] = [i]
    for parola in list(D.keys()):
        if len(D[parola]) < K:
            del D[parola]

    coppie = [(k, len(v)) for k, v in D.items()]
    coppie.sort(key=lambda coppia: (-coppia[1], coppia[0]))
    with open(path_out, mode="w", encoding="utf8") as FOUT:
        for parola, numero in coppie:
            print(parola, numero, file=FOUT)
    return D
...
I quadretti 2x2 devono apparire almeno 2 volte.
...
def func5(path_png_in: str) -> dict[str, set[tuple[int, int]]]:
    ...
    for y in range(1, len(img) - 1):
        for x in range(1, len(img[0]) - 1):
            if (
                b != img[y][x] == img[y][x + 1] == img[y + 1][x] == img[y + 1][x + 1]
                and b
                == img[y][x - 1]
                == img[y][x + 2]
                == img[y - 1][x]
...
def ex1(radice: tree.BinaryTree, lista_pesi: list[int]) -> tree.BinaryTree:
    pass
    # completa la funzione
    return appoggio(radice, lista_pesi, 0)


def appoggio(root, lp, livello):
    if root is None:
        return None
    SX = appoggio(root.left, lp, livello + 1)
    DX = appoggio(root.right, lp, livello + 1)
    return tree.BinaryTree(root.value * lp[livello], SX, DX)


def ex1_2(root: tree.BinaryTree, lp: list[int]) -> tree.BinaryTree:
    if root is None:
        return None
    SX = ex1(root.left, lp[1:])
    DX = ex1(root.right, lp[1:])
    return tree.BinaryTree(root.value * lp[0], SX, DX)
...
def ex2(path: str, lista_estensioni: list[str]) -> dict[str, list[str]]:
    pass
    # completa la funzione
    D = {est: set() for est in lista_estensioni}
    for nome in os.listdir(path):
        fullname = path + "/" + nome
        if os.path.isdir(fullname):
            sottosoluzione = ex2(fullname, lista_estensioni)
            for k, insieme in sottosoluzione.items():
                D[k] = D[k].union(insieme)
        else:
            for e in lista_estensioni:
                if nome.endswith(e):
                    D[e].add(path)
    return D
