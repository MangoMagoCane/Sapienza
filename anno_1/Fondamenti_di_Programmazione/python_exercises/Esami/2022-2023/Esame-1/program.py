...
def func1(string_list: list[str], word: str) -> int:
...
def func2(pathname: str) -> dict[str, int]:
    dct: dict[str, int] = {}
...
def func3(listaA: list[str], pathname: str) -> int:
    char_count: int = 0
    lst_sorted = sorted(listaA, key=lambda e: (-len(e), e), reverse=True)
...
def func4(S: str) -> str:
    ...
    print(sorted(freq_dct))
    hist_str: str = ""
    for s in sorted(freq_dct):
        hist_str += s.ljust(max_len + 1) + freq_dct[s] * "*" + "\n"
    return hist_str
...
def ex1(root: str):
    dct: dict[str, str] = {}
    for l in map(lambda l: root + "/" + l, os.listdir(root)):
        if os.path.isdir(l):
            print(l)
        elif os.path.isfile(l) and l.endswith(".txt"):
            dct[l[:-4]] = fileUni(l)

    print(dct)


def fileUni(filepath: str) -> str:
    output: str = ""
    with open(filepath, "r") as f:
        for line in f.readlines():
            codes: list[str] = line.split(" ")
            for code in codes:
                output += chr(int(code.strip()))
                pass
    return output
...
def ex2(nums: set[int], ops: list[str]):
    # write here you code
    pass
...
if __name__ == "__main__":
    ...
    print(
        "Otherwise you can insert here you code to test the functions but you have to write your own tests"
    )
    print("*" * 50)
