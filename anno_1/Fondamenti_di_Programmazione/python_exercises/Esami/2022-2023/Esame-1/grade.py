...
def test_personal_data_entry():
    if "name" in program.__dict__:
        assert (
            program.name != "NAME"
        ), f"{COL['YELLOW']}ERROR: Please assign the 'name' variable with YOUR NAME in program.py{COL['RST']}"
        assert (
            program.surname != "SURNAME"
        ), f"{COL['YELLOW']}ERROR: Please assign the 'surname' variable with YOUR SURNAME in program.py{COL['RST']}"
        assert (
            program.student_id != "MATRICULATION NUMBER"
        ), f"{COL['YELLOW']}ERROR: Please assign the 'student_id' variable with YOUR MATRICULATION NUMBER in program.py{COL['RST']}"
    else:
        assert (
            program.nome != "NOME"
    ...
    if res != expected:
        ...
        if keys_ex == keys_rez:
            for k in expected:
                if expected[k] != res[k]:
                    print(
                        f"""[ERROR] Ad esempio, la matricola {k} dovrebbe avere una il valore {expected[k]} invece che {res[k]}."""
                    )
                    print(
                        f"""[ERROR] For example, matr {k} should have a value of {expected[k]} instead of {res[k]}."""
                    )
                    return 0
                else:
                    pass
            ...
            if diff_ex:
                print(
                    f"""[ERROR] Le chiavi nei dizionari non sono uguali: al tuo dizionario mancano le matricole {diff_ex}"""
                )
                print(
                    f"""[ERROR] Keys in the dictionaries are not equal: your dictionary misses the following matr  {diff_ex}"""
                )
            diff_rez = keys_rez - keys_ex
            if diff_rez:
                print(
                    f"""[ERROR] Le chiavi nei dizionari non sono uguali: il tuo ha in piu le seguenti matricole {diff_rez}"""
                )
                print(
                    f"""[ERROR] Keys in the dictionaries are not equal: the following matr are not necessary {diff_rez}"""
                )
            return 0
    return 2 / 3
...
def do_func3_tests(listA, expected, outpath):
    ...
    if os.path.isfile(actual_out_path):
        try:
            testlib.check_text_file(actual_out_path, expected_path)
        except:
            print(
                f"{'*'*50}\n[ERROR] Il tuo file è\n --> {actual_out_path} \n VS. il file atteso\n --> {expected_path}\n {'*'*50}",
                end="\n",
...
def do_test_ex1(directory, expected):
    ...
    testlib.check(
        res, expected, None, f"Wrong list! Returned={res}, expected={expected}"
    )
    return 2
...
def test_ex1_2():
    ...
    directory = "ex1_B"
    expected = {
        "ex1_B/stSOl": "àWŰĦÁƭĉǌǀ",
        "ex1_B/vWHJp": "IřǱāƁƢÒ~ǀď\x85¹Ɩƕ",
        "ex1_B": "¶\x9d\x83ÔǄď",
    }
    return do_test_ex1(directory, expected)
...
def test_ex1_3():
    ...
    directory = "ex1_C"
    expected = {
        "ex1_C/Lospm/Khvrj": "ª\x84ĵķƉðĔMǇǧƴĈ",
        "ex1_C/Lospm/TTINY": "®ªÖŇTƑǁĮĊvƅĕŒ",
        "ex1_C/Lospm/nghch": "ØÜÄeŽêíƍōWŚ",
        "ex1_C/Lospm/vcynO": "Ŏ«\x83ı²qŐ",
        "ex1_C/Lospm/wGuOG": "ŖõƙċƂ³¨",
        "ex1_C/Lospm": "Ơ\\",
        "ex1_C/MfzQL/eiKqi": "¿ÜwþǟL",
        "ex1_C/MfzQL/fNcLp": "wĥ¿ŷ\x81",
        "ex1_C/MfzQL/rtGJs": "ſŻÑǙ³Ƃ\x84ƬƁ\x9f",
        "ex1_C/MfzQL": "ŕŰťǋ\xad²\x9aœ",
        "ex1_C/sOsTK/Ffufy": "\x97ņíƢ",
        "ex1_C/sOsTK/IpUQx": "ƼŦĲűų",
        "ex1_C/sOsTK/pjDEZ": "^ǃuŽ»ų",
        "ex1_C": "ņĞö\x88ŭ",
    }
    return do_test_ex1(directory, expected)
...
def do_ex2_test(strings, n, expected, score=2):
    ...
    if not isinstance(res, (set)):
        testlib.check(
            ...
            set,
            None,
            f"Wrong return type! Returned={type(res)}, expected= set / Il tipo ritornato non è corretto! Ritornato={type(res)}, atteso= set",
        )
    p = 0
    if type(res) == set:
        testlib.check(
            ...
            set(expected),
            None,
            f"Wrong set returned! Returned={res}, expected={expected} / Il set ritornato non è corretto! Ritornato={res}, atteso ={expected}",
        )
...
if __name__ == "__main__":
    ...
    constraint1 = (
        len(
            [
                ...
                if grade > 0 and name.startswith("func")
            ]
        )
        >= 3
    )
    constraint2 = (
        len(
            [
                name
                for name, grade in grades.items()
                if grade > 0 and name.startswith("ex")
            ]
        )
        >= 1
    )
    ...
    if not constraint1:
        print(
            f'YOU HAVE NOT PASSED AT LEAST 3 FUNC EXERCISES: {COL["RED"]}EXAM NOT PASSED{COL["RST"]}'
        )
    elif not constraint2:
        print(
            f'YOU HAVE NOT PASSED AT LEAST 1 RECURSIVE EXERCISE: {COL["RED"]}EXAM NOT PASSED{COL["RST"]}'
        )
    elif not constraint3:
        print(
            f'THE FINAL GRADE IS LESS THAN 18: {COL["RED"]}EXAM NOT PASSED{COL["RST"]}'
        )
    else:
        print(
            f"YOU HAVE {COL['GREEN']}PASSED{COL['RST']} THE EXAM WITH {COL['GREEN']}{total}{COL['RST']} POINTS"
        )
    ENDC = "\033[0m"
    BOLD = "\033[1m"
    print(
        f"Three func problems solved:  {COL['BOLD']} {COL['GREEN'] if constraint1 else COL['RED']} {constraint1}{COL['RST']}{COL['ENDC']}"
    )
    print(
        f"One ex problem solved:       {COL['BOLD']} {COL['GREEN'] if constraint2 else COL['RED']} {constraint2}{COL['RST']}{COL['ENDC']} "
    )
    print(
        f"Total >= 18:                 {COL['BOLD']} {COL['GREEN'] if constraint3 else COL['RED']} {constraint3}{COL['RST']}{COL['ENDC']}"
    )
    print(
        f"Exam passed:                 {COL['BOLD']} {COL['GREEN'] if constraint4 else COL['RED']} {constraint4}{COL['RST']}{COL['ENDC']}"
    )
################################################################################
