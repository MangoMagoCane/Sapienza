...
def func5(points: list[tuple[int, int]]):
    lst = sorted([(math.sqrt(p[0] ** 2 + p[1] ** 2), i) for i, p in enumerate(points)])[
        :3
    ]
    ...
    print("foo", lst)
    return (round(xsum / 3.0, 3), round(ysum / 3.0, 3))
...
def ex1(root: BinaryTree, result: list[int]):
    lst: list[tuple[int, int]] = rec(root, 0)
    dct: dict[int, int] = {}
    for e in lst:
        if e[1] in dct:
            dct[e[1]] += e[0]
    ...
    print([dct[i] for i in sorted(dct.keys())])


def rec(root: BinaryTree, depth: int):
    lst: list[tuple[int, int]] = [(root.value, depth)]
    if not root.left == None:
        lst += rec(root.left, depth + 1)
    if not root.right == None:
        lst += rec(root.right, depth + 1)
...
def ex2(dirin: str, words: list[str]) -> list[tuple[str, int]]:
    freq_lst: list[tuple[str, int]] = []
    tmp_dct: dict[str, int] = recurs(dirin, words)
    # print(tmp_dct)
    return sorted([(k, v) for k, v in tmp_dct.items()], key=lambda p: (p[1], p[0]))
...
def recurs(dirin: str, words: list[str]) -> dict[str, int]:
    ...
    for filename in map(lambda n: dirin + "/" + n, os.listdir(dirin)):
        if os.path.isfile(filename) and filename.endswith(".txt"):
            ...
            # print("bar", filename)
            mergedicts(tmp_dct, recurs(filename, words))

    return tmp_dct


def mergedicts(dct1: dict[str, int], dct2: dict[str, int]):
    for key in dct1:
        dct1[key] += dct2[key]


def handlefile(filename: str, words: list[str]) -> dict[str, int]:
    tmp_dct: dict[str, int] = {w: 0 for w in words}
